            ...
            if ($pet->status === 'lost') {
                $clues = [
                    [
                        ...
                        'description' => '看到一只类似的宠物在垃圾桶旁边找吃的，看起来很饿的样子',
                        'is_private' => false,
                    ...
                    [
                        ...
                        'address' => '东城区五四大街人行道旁',
                        'seen_at' => $pet->last_seen_at->addHours(4),
                        'description' => '在路边看到过，当时正在过马路，好像在找什么',
                        'is_private' => false,
                        'user_id' => $users[($index + 2) % $users->count()]->id,
                    ],
                    [
                        'lat' => $pet->last_seen_lat + 0.008,
                        'lng' => $pet->last_seen_lng - 0.004,
                        'address' => '东城区东四北大街XX号',
                        'seen_at' => $pet->last_seen_at->addHours(7),
                        'description' => '在我们小区看到过，有人试图接近但它跑开了',
                        'is_private' => false,
                        'user_id' => $users[($index + 0) % $users->count()]->id,
                    ],
                    [
                        'lat' => $pet->last_seen_lat - 0.006,
                        'lng' => $pet->last_seen_lng - 0.001,
                        'address' => '东城区王府井大街XX小区院内',
                        'seen_at' => $pet->last_seen_at->addHours(10),
                        'description' => '在小区院子里看到过，当时有个穿灰衣服的人在附近，可能是宠物的家人',
                        'is_private' => true,
                        'user_id' => $users[($index + 1) % $users->count()]->id,
                    ],
                ];
