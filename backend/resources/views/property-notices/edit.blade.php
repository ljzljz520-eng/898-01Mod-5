...
@section('title', '编辑物业公告')

@section('content')
<div class="max-w-4xl mx-auto">
    <h1 class="text-3xl font-bold text-gray-800 mb-6">编辑物业公告</h1>
    
    <div class="card">
        <form method="POST" action="{{ route('property-notices.update', $topic) }}" id="property-notice-form">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <select id="notice_type" name="notice_type" required
                    ...
                    @foreach($noticeTypes as $key => $name)
                        <option value="{{ $key }}" {{ old('notice_type', $topic->notice_type) == $key ? 'selected' : '' }}>{{ $name }}</option>
                    @endforeach
                </select>
                @error('notice_type')
            ...
            <div class="mb-4">
                ...
                <textarea id="content" name="content" rows="12" required
                          ...
                          placeholder="请输入公告内容...">{{ old('content', $topic->content) }}</textarea>
                @error('content')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                <label for="change_summary" class="block text-gray-700 text-sm font-medium mb-2">修改说明（可选）</label>
                <input type="text" id="change_summary" name="change_summary" value="{{ old('change_summary') }}"
                       class="input-field @error('change_summary') border-red-500 @enderror"
                       placeholder="简要说明本次修改的内容，方便居民了解变更">
                <p class="mt-1 text-xs text-gray-500">修改后系统会自动保存历史版本，居民可查看新旧差异</p>
                ...
                <div class="bg-gray-50 rounded-lg p-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3" id="buildings-container">
                        @foreach($buildings as $building)
                            @php
                                $buildingSelected = $selectedRecipients->where('type', 'building')->where('value', $building['name'])->isNotEmpty();
                            @endphp
                            <div class="building-item bg-white border border-gray-200 rounded-lg p-3">
                                <label class="flex items-start gap-2 mb-2">
                                    <input type="checkbox" class="building-checkbox rounded border-gray-300 text-primary-600 focus:ring-primary-500" 
                                           data-building="{{ $building['name'] }}"
                                           {{ $buildingSelected ? 'checked' : '' }}>
                                ...
                                @if(!empty($building['units']))
                                    <div class="ml-6 mt-2 space-y-1">
                                        @foreach($building['units'] as $unit)
                                            @php
                                                $unitValue = $building['name'] . '-' . $unit;
                                                $unitSelected = $selectedRecipients->where('type', 'unit')->where('value', $unitValue)->isNotEmpty();
                                            @endphp
                                            <label class="flex items-center gap-2">
                                                <input type="checkbox" class="unit-checkbox rounded border-gray-300 text-primary-600 focus:ring-primary-500" 
                                                       ...
                                                       data-unit="{{ $unit }}"
                                                       value="{{ $unitValue }}"
                                                       {{ $buildingSelected || $unitSelected ? 'checked' : '' }}>
            ...
            <div class="mb-6">
                <label class="flex items-center gap-2">
                    <input type="checkbox" id="is_pinned" name="is_pinned" class="rounded border-gray-300 text-primary-600 focus:ring-primary-500" {{ old('is_pinned', $topic->is_pinned) ? 'checked' : '' }}>
    ...
    <div class="mt-6">
        <a href="{{ route('property-notices.version-history', $topic) }}" class="text-primary-600 hover:text-primary-700">
            → 查看版本历史
        </a>
    </div>
</div>
